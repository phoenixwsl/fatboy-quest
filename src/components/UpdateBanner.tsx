...
export function UpdateBanner() {
  ...
  return (
    <div
      className="fixed top-0 left-0 right-0 z-[200] px-4 py-3 shadow-lg"
      style={{
        background: 'linear-gradient(90deg, var(--state-success), var(--state-info))',
        color: '#fff',
      }}
    >
      <div className="max-w-md mx-auto flex items-center gap-3">
        ...
        <div className="flex-1 text-sm">
          ...
          <div className="text-xs" style={{ color: 'rgba(255,255,255,.85)' }}>点更新立即生效</div>
        </div>
        <button
          onClick={() => updateServiceWorker(true)}
          className="px-4 py-2 rounded-xl font-bold text-sm active:scale-95"
          style={{ background: 'var(--surface-paper)', color: 'var(--state-success-strong)' }}
        >
          更新
        </button>
        <button
