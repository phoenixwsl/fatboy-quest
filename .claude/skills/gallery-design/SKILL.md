...
## 3b. 画框比例 + 摆放原则(R5.8.0 沉淀)

> 用户提了一个好问题:"哪些是最优的画框比例,如何摆放?" 这一节是研究后的沉淀。

### 为什么是这 4 个比例(不是其他)

| 比例 | 出处 / 经典使用 | 为什么选 |
|---|---|---|
| **1:1 方形** | Hasselblad 中画幅、Polaroid Square、Instagram 默认 | 平衡、无方向性,适合人像/静物/抽象/印章;最 fool-proof 的构图 |
| **4:5 拍立得** | 8×10 portrait studio print、Polaroid SX-70 | 经典相纸比例,**最温馨/亲密**;**默认推荐** |
| **4:3 横幅** | 经典 TV / 拍照默认 / 微 4/3 系统 | 风景/合影最自然;snapshot 最常见 |
| **3:4 竖幅** | 智能手机竖拍、油画肖像传统 | 人像/单品 |

### 为什么**不**选这些(对比讨论)

| 比例 | 拒绝理由 |
|---|---|
| **3:2 / 2:3** (35mm 全画幅) | 太窄太宽,跟 4:5 视觉差异不大却没 4:5 的"亲密感" |
| **16:9** | 太电影感,不读为"框装艺术品",而是"屏幕" |
| **A4 (1:√2)** | 太"文件",书页感 |
| **21:9 全景** | 长卷,在 masonry 里排版尴尬 |
| **黄金比 1:1.618** | 理论上美,但实际跟 4:5 + 3:2 视觉差异小,用户辨识度低 |

**4 个就够,不要 8 个**。选择多 = 决策疲劳,孩子上传时摩擦增加。

### 摆放原则(salon hang vs grid)

真正的画廊不用 Pinterest 那种均匀网格 —— 用的是 **salon hang**(沙龙挂画,19 世纪美术馆传统):

1. **错落出节奏**:同形状不要连排 5 张;大小穿插制造视觉节拍
2. **锚 + 卫星轻量**:大画当焦点,小画簇拥,但不严格分层(孩子作品同等重要)
3. **留白 = 重要信号**:大画周围留更多空间,放大它的存在
4. **不要齐边**:卡片左右上下都不严格对齐,留 hash-based 错位
5. **光学平衡,不对称**:视觉重量要平衡,但不要镜像式严格对称(institutional/冷)
6. **微倾斜暗示手作**:每幅 ±1.5° 随机旋转,模拟"手贴"vs"机器排"
7. **不按颜色聚类**:家庭照片是混合主题,强行聚类会失真

### 实现映射(代码层)

| skill 原则 | 实现在哪 |
|---|---|
| 微旋转 ±1.5° | `galleryLayout.ts::layoutVariance` |
| 列内宽度变化(大/小档)| `GalleryImage.displaySize` + `layoutVariance` |
| 错位对齐 | CSS `data-align` + `margin-left/right: auto` |
| 阴影深浅变化(暖光)| `layoutVariance.shadowAlpha` + inline `box-shadow` |
| gap 大留白 | `gallery.css` `.gallery-masonry` `column-gap: 28-32px` |
| 不按颜色聚类 | (无显式实现 —— 排序按 uploadedAt,不按色彩) |
| 锚 + 卫星 | 用户选 `displaySize: large/small` |
...
### 法则 4:微旋转 ±1.5° + 列内宽度变化 + 错位对齐(**R5.8.0 改为默认开启**)
- 每张卡片基于 `image.id` 的稳定 hash 给一个微旋转角度(±1.5°,30% 概率取 0 保持端正)
- 同一图在同一位置:旋转/对齐/padding 完全 deterministic —— 刷新不跳
- 卡片宽度 78%-100% 在列内浮动(由 displaySize + hash 决定,见法则 4b)
- align: left / right / center 三种,让卡片不严格齐边
- 整体感觉像"手贴的相片墙",不像 Pinterest 严格 grid
- 关掉的代价:页面立刻"很 app"

**R5.8.0 调整理由**:原"默认关闭、墙上预览才开"违反了 Cosmos.so 调性("瀑布流但不拥挤、错落、有呼吸"),用户反馈"太规整了" → 改为默认开。

### 法则 4b:画框大小两档 `large / small`(salon hang 灵魂)
真正的"画展感"不是靠均匀错落,而是靠**大画/小画穿插**(美术馆 salon hang 传统):
- `large`:列内宽度 0.85-1.0,视觉锚点,带读者的眼睛
- `small`:列内宽度 0.55-0.72,卫星图,给 large 旁边留出呼吸
- 用户上传时选 + 后续可"换画框"重选
- 默认 `large`(显眼优先,小是 opt-in)
- **不要自动决定大小** —— 让用户用大小表达"这张对我更重要"
...
| = 100 | 上传按钮 disabled,虚线卡片改文案:"画廊已满,请取下几张老作品再来" |

### 6.4 上传编辑器规范 (R5.8.0+)

上传不再是"选图 → 直接入库",而是走 GalleryUploadEditor:

**两步:**
1. **画框 + 大小 + 旋转 + 平移**(crop 阶段)
2. **元数据表单**(meta 阶段,全 optional)

**画框选择 — 4 个固定比例,无"原图"option**(整体画展感统一):
- 方形 1:1
- 拍立得 4:5(默认)
- 横幅 4:3
- 竖幅 3:4

**大小选择 — 2 档**:`large` / `small`(见法则 4b)

**触屏交互**:
- 图片填满预览框 + cover mode 自动 fit 到画框比例
- 用户**单指拖动**调整位置(平移,不缩放)
- 旋转 90° 步进按钮(每点击 +90°)
- 不支持自由比例/自由角度/滤镜/贴纸(见反 feature 列表)

**元数据表单**:
- title / artist / year / medium / caption,全 optional
- artist 默认填 `settings.childName`
- year 默认填当前年
- caption ≤ 80 字
- 返回"改画框"按钮可回到 crop 阶段
- "挂上去"按钮提交

**保留 originalBlob**:
- 长边 1600 JPEG 0.85(~600KB),用于后续"换画框"
- 老图(R5.7 时期)无 originalBlob,"换画框"按钮 disable
...
| **删除/取下** | ✓(二次确认弹窗) | ✓(同) |
| **换画框** | ✓ (需 originalBlob,R5.8+ 上传的图都有) | ✓ |
| **换描述** | ✓(可改自己上传的,家长可改任何)| ✓ |
| 管理模式(卡片角标 × 批量取下) | ✓ | ✓ |

**R5.8.0 调整**:原"仅家长可删"被用户判断后改为"双端可删,二次确认护栏依然在"。
- 风险:孩子冲动控制弱,可能误删家长上传的家庭照片
- 缓解:删除走"再想想 / 取下"二次确认 modal,默认 focus "再想想"
- 设计理由:**先信任后修正**(扩展到删除)。孩子上传 immediately 可见获得即时反馈;删除有摩擦但不阻断;家长可监控(后续加 Bark push 提醒)。
- **如果娃乱删成常态**,在 ParentSettings 加"删除需家长 PIN"开关回退到旧模式
...
| **自由比例裁剪(任意 ratio + drag handles)** | 0 | 0 | -2 | 0 | **-2** | 4 固定画框就够,自由比例破坏画展整体一致性 + 触屏 UX 复杂 |
| **图片滤镜(黑白/复古/HDR/AI 风格)** | 0 | -1 | -1 | 0 | **-2** | 削弱原始作品真实感,Instagram 化 |
| **贴纸/文字水印** | 0 | 0 | -1 | 0 | **-1** | 画廊不是社交媒体,加贴纸破坏画展感 |
| **任意角度自由旋转** | 0 | 0 | -1 | 0 | **-1** | 90° 步进足够,自由角度只增加 cognitive load |
